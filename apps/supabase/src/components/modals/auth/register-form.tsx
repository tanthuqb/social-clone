...
export default function RegisterForm() {
  ...
  const signInWithProviderOAuth = async (provider: Provider) => {
    ...
    if (error) {
      console.error(
        `Error signing in with ${provider.toString()}:`,
        error.message,
      );
      return;
    }
    ...
    <>
      <div className="flex flex-col  gap-2.5">
        ...
        <button
          ...
          onClick={() => {
            signInWithProviderOAuth("google");
          }}
        >
        ...
        {/* === Button Facebook ==== */}
        <button className="flex w-full flex-col items-start gap-1 self-stretch rounded-full border border-slate-100 p-2">
          <div className="flex w-full gap-2">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="21"
